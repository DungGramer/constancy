...
// Stable alphabetical sort for fingerprinting. Uses localeCompare with a fixed
// 'en-US' locale + variant sensitivity so results are consistent across machines
// (default-locale calls would vary by runtime environment).
export const _fingerprintSort = (a: string, b: string): number =>
  a.localeCompare(b, 'en-US', { sensitivity: 'variant' });
